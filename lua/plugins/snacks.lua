...
return {
  {
    ...
    keys = {
      ...
      { "<leader>fl", function() Snacks.picker.lazy() end,               desc = "Lazy Plugin Specs" },
      ...
      -- Other
      { "<leader>H", function() Snacks.notifier.show_history() end, desc = "Open History" },
    ...
    opts = {
      ...
      gh = { enabled = false }, -- github
      gitbrowse = { enabled = true }, -- NOTE: add url_patterns if not a known/public repo
      image = { enabled = false },
      indent = { enabled = true, animate = { enabled = false } },
      scratch = { enabled = true, name = "Scratchpad" },
      -- Snacks styles
      ---------------------------------------------------------------
      styles = {
        input = { relative = "cursor" },
        notification = { wo = { wrap = true } },
        notification_history = { width = 0.8, height = 0.8 },
      },
      toggle = {
        color = {
          disabled = "grey",
    ...
    init = function()
      vim.api.nvim_create_autocmd("User", {
        ...
        callback = function()
          ...
          Snacks.toggle.option("relativenumber")              :map("<leader><leader>L")
          Snacks.toggle.option("conceallevel", conceal_opts)  :map("<leader><leader>c")
          Snacks.toggle.option("background", background_opts) :map("<leader><leader>b")
          Snacks.toggle.line_number({ name = "number" })      :map("<leader><leader>l")
          Snacks.toggle.diagnostics()                         :map("<leader><leader>d")
          Snacks.toggle.animate()                             :map("<leader><leader>a")
